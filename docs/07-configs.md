...
5. 进度：TaskCenter 轮询（Q9/Q39）；遮罩展示精简阶段 `hostname · 阶段` 与实时结果树（Q104）；完成摘要 `detail` 为「N 新增, M 更新, D 删除」，结果树含新建/更新/删除项；失败标黄跳配置列表主机名过滤；状态链任务详情（Q8）；同步线程异常时任务标 `failed`，避免遮罩卡住（Q103）。
