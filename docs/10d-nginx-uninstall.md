...
进入第 2 步探测 `nginx -V`：多节点按 `node.batch_max_count` **并行**探测；单节点尽量 **一次 SSH** 完成 `-V`、包归属、托管方式与运行态。提取 `--prefix` 与名含 `-path` 的绝对路径参数；另追加设置项。同时探测包归属（`install_origin` / `package_name`）。
...
收敛后 **同路径精确去重**（常见：全在 `--prefix=/opt/app/nginx` 下时探测侧只剩必选 `--prefix`）。

### 4.2 父子路径去重与勾选锁定

最终删除集合若同时含父目录与子路径（如 `/etc/nginx` 与 `/etc/nginx/conf.d/test.conf`），只保留最外层，避免先删父目录再删子路径。

探测页：**仍展示**真包含的子路径；父路径勾选后子集自动勾选且不可取消（徽标「随父路径」）；取消父勾选后子集解锁并同步取消勾选。`--prefix` 必选恒勾，其下路径始终随父锁定；编辑 prefix 后重算。包占用 / 必选规则优先于包含联动。

探测路径列表支持点击整行切换勾选（必选 `--prefix`、包占用、随父锁定除外）。
## 5. 执行进度
