...
- 日期：2026-08-19
- 状态：已完成（2026-08-20）
- 关联台账：[`AGENTS.md`](../../AGENTS.md) Q162

## 实施结果

- P1：凭证启用任务在 SSH 成功后同步探测并写入节点 Nginx 状态。
- P2–P4：`split_search_tags` 统一中英文逗号、多标签 AND；节点、配置、用户/角色、发布/任务及运维历史等接线完成。
- P5：所有使用统一 tag-input 的列表改为 `fetch` 局部替换主内容区与 `pushState`；失败保留整页 GET 回退；页面替换后重新初始化交互。
- P6：配置列表先数据库筛选/分页，仅预取当前页绑定与分组；状态计数改一次聚合；多 Accordion 恢复展开时不播放批量动画。
- 验证：Django system check 与全量 78 项测试均通过。发布 API 测试夹具显式标记 Nginx 可用，保持 Q150 门禁。
