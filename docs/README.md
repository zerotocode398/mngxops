...
本文档集基于**当前代码实现**倒推整理（实现即基线需求），按功能模块拆分。历史交互优化结论见仓库根目录 [`AGENTS.md`](../AGENTS.md)（Q1–Q122 结论台账）。
...
- **AGENTS.md**：优化点结论台账（Q1–Q122）的**唯一**来源；已完成项沉淀为 docs 中的「已确认行为」。
- 未实现或已关闭能力：记入 `AGENTS.md`（续编 Q122+），不在模块正文写成已交付。
