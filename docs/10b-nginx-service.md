...
- 批次格式：`OP-YYMMDD-NNNN`（写入 `TaskCenterTask.source_batch`）。
- 启停历史 `?search=` 支持主机名 / IP / 动作 / 明细 / **批次**；列表含批次列。
- 任务中心详情「来源批次」、首页「最近启停任务」批次可跳转 `/nginx-service/history/?search=`。
...
- [`apps/nginx_service/views.py`](../apps/nginx_service/views.py)（含 `generate_service_batch_number`）
