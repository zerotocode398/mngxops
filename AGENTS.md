...
| Q103 | 解除绑定文案/弹窗宽度 + marked_deleted 同步卡住 | 已完成 |
| Q104 | 同步进度遮罩精简阶段 + 实时结果树 | 已完成 |
...
| Q100 | 状态标签只滤节点导致明细混入其它状态 | 已完成 |
| Q101 | 发布临时中转文件残留未清理 | 已完成 |
| Q102 | 发布成功后确认清单仍展示已发配置 | 已完成 |
| Q106 | 发布历史/详情版本徽标点击预览内容 | 已完成 |
| Q107 | 回滚页版本预览对齐发布历史列表 + 绑定字段修正 | 已完成 |
| Q114 | 离线/未知节点禁止同步与发布/回滚 | 已完成 |
...
| Q71 | 摘要与结果树协议统一 | 已完成 |
| Q105 | 同步结果树含跳过明细 + 任务详情默认展开 | 已完成 |
| Q109 | 任务中心协作式取消（断 SSH + 检查点） | 已完成 |
| Q110 | 筛选自动查询 + 表格宽度 + 操作 icon-only | 已完成 |
| Q111 | 列表无横向滚动 + 摘要窄列（全文看详情） | 已完成 |
| Q112 | 列表摘要主机最多展示 3 台 | 已完成 |
| Q115 | 详情失败原因管道符还原多行展示 | 已完成 |
...
| Q116 | 无权访问回首页 + showAlert 弹窗 | 已完成 |
| Q117 | 权限矩阵无法勾选 / 默认误显「取消」 | 已完成 |
...
| Q73 | 侧栏可折叠 | 已完成 |
| Q81 | 升级设置生效时机修复 | 已完成 |
| Q108 | 首页改版：最近任务 TaskCenter + 紧凑入口 | 已完成 |
...
| Q108 | 首页改版：最近任务 TaskCenter + 紧凑入口 | 已完成 |
| Q109 | 任务中心协作式取消（断 SSH + 检查点） | 已完成 |
| Q110 | 任务中心筛选自动查询 + 表格宽度 + 操作图标 | 已完成 |
| Q111 | 任务中心无横向滚动 + 摘要窄列（全文看详情） | 已完成 |
...
- **问题**：状态标签只过滤「含该状态绑定」的节点；展开/勾选仍用节点下全部绑定，导致「待推送」含已同步、「已同步」含待推送（失败/远程删除同理）。
...
- **问题**：备份已按 `release.backup_dir/{hostname}/` 落盘正确；上传中转 `/tmp/*.mngxops_tmp` 校验后未删除，易被误认为备份路径异常。
...
- **问题**：发布任务成功后 Step2「确认发布清单」仍保留已选绑定，可重复展示/误再发。
- **处理**：`publishBindings` 记录 `lastPublishedBindingIds`；进度轮询 `finished && status===success` 时 `removePublishedFromSelection` 剔除、同步 Step1 勾选、重建预览；清单空则回 Step1。部分失败本轮保留清单便于重试。
- **状态**：已完成
...
- **问题**：
  ...
  3. 标记删除后再同步：`_ensure_binding` 排除 `marked_deleted` 后仍 create，触发 `(config,node)` unique 冲突；线程未捕获异常，TaskCenter 停在 `running`，进度遮罩卡住。
- **处理**：
...
- **问题**：单/批同步遮罩仅一行 `detail`，无发布式精简阶段与实时结果树；进度 API 无 `releases.read` 时不返回 `config_batch_sync`。
- **处理**：
...
- **问题**：同步详情仅见「已同步 N 个配置文件（新增 0…）」汇总；全跳过时结果树无配置名；成功节点默认折叠。
- **处理**：
...
- **问题**：回滚页页头「当前版本」徽章不可点；表格高亮误用 `config.current_version`、路径误用 `config.file_path`（二者属 binding）；点击版本号无法像发布历史一样弹出配置预览。
- **处理**：
  ...
  3. 版本预览 JS **对齐发布历史 list.html**（独立 `<script>`、`preventDefault`+`stopPropagation`、同弹窗同 API）；修复 `buildNodeGroup` 三元表达式缺 `)` 导致整页脚本无法解析、预览从未绑定的问题。
  4. `ReleaseRollbackView.get` 补 `select_related("version")`。
- **状态**：已完成

### Q108 · 仪表盘 · 首页改版

- **问题**：首页「配置下发失败」冗余；「最近发布任务」仅 `ReleaseTask` 与「查看全部→任务中心」不一致；「快捷操作」大卡片风格突兀；缺执行中/失败等运维信号。
- **处理**：
  ...
  3. 去掉大卡片快捷操作，页头改为紧凑 `btn-sm`（发布中心/配置列表/新建节点/任务中心）。
...
- **问题**：历史异常导致部分任务长期「执行中」，列表无取消入口；执行线程不读 `cancelled`。
- **处理**：
  ...
  4. **不做** 远程/本地 `kill -9`；断连后远程进程可能残留。
- **状态**：已完成

### Q110 · 任务中心 · 列表筛选与操作列

- **问题**：类型/状态下拉切到「全部」不自动查询；表格过宽需横向拖动；操作列「详情/取消」带文字，与其它列表 icon-only 不一致。
- **处理**：
...
- **问题**：列表仍出现底部横向滚动条；摘要列过宽（38%），长文案应引导进详情查看。
- **处理**：
...
- **问题**：停在第 3 步编译参数后离开再进升级中心，会自动回到第 3 步并带上草稿，不符合「开始升级应从选择目标」的预期。
- **处理**：`restoreUpgradeWizardState` 强制第 1 步；**不恢复**节点/源码包/编译参数等草稿；`sessionStorage` 仅保留进行中批次进度以便续看；去掉为恢复第 3/4 步预拉 `nginx -V`。
- **状态**：已完成

### Q114 · 节点 · 离线禁止同步与发布

- **问题**：`status` 为 `offline`/`unknown` 时仍可配置同步、发布中心发布及历史回滚，与升级中心「仅 online」门禁不一致。
- **处理**：
...
- **问题**：升级等失败原因经 `short_error_tail` 用 ` | ` 压成单行后，详情执行结果平铺难读。
