...
class DiscoverNginxConfigsTests(SimpleTestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls._ssh_patcher = patch("utils.ssh.SSHClient", autospec=True)
        cls._mock_ssh_cls = cls._ssh_patcher.start()
        cls._mock_ssh_cls.return_value.connect.return_value = (True, None)

    @classmethod
    def tearDownClass(cls):
        cls._ssh_patcher.stop()
        super().tearDownClass()
