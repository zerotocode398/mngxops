...
PRESET_SETTINGS = [
    ...
    {"key": "node.ssh_connect_timeout", "group": "节点管理", "type": "integer", "value": "10",
     "label": "SSH 连接超时（秒）",
     "description": "所有 SSH 远程操作的连接超时时间（下次连接立即生效）", "sort_order": 11},
    {"key": "node.ssh_default_port", "group": "节点管理", "type": "integer", "value": "22",
     "label": "SSH 默认端口",
     "description": "仅影响新建节点/批量导入时的默认 SSH 端口，已有节点不改动", "sort_order": 12},
    {"key": "node.detect_retries", "group": "节点管理", "type": "integer", "value": "1",
     "label": "节点探测重试次数",
     "description": "SSH 连接失败时的额外重试次数（不含首次；下次连接立即生效）", "sort_order": 13},
    # 凭证管理
    {"key": "credential.test_max_concurrency", "group": "凭证管理", "type": "integer", "value": "10",
     "label": "凭证测试最大并发数",
     "description": "凭证启用批量测试的最大并发数（下次启用测试立即生效）", "sort_order": 20},
    # 配置管理
    ...
    {"key": "upgrade.package_max_size_mb", "group": "Nginx升级", "type": "integer", "value": "500",
     "label": "源码包上传大小限制 (MB)",
     "description": "上传校验立即按新限制生效；", "sort_order": 82},
]
