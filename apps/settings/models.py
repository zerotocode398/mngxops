...
PRESET_SETTINGS = [
    ...
    # OpenSSH 升级
    {"key": "openssh.default_work_dir", "group": "OpenSSH升级", "type": "string", "value": "/tmp/openssh-upgrade",
     "label": "默认编译工作目录",
     "description": "OpenSSH 升级向导默认远程编译目录；保存后刷新向导生效",
     "sort_order": 100},
    {"key": "openssh.backup_dir", "group": "OpenSSH升级", "type": "string", "value": "/opt/app/mascloud/ansible/mngxops/openssh",
     "label": "远程备份根目录",
     "description": "升级前备份二进制与 /etc/ssh 的远端根目录，实际路径为 {backup_dir}/{节点hostname}/openssh/{时间戳}/；回滚从此恢复。保存后下次执行生效",
     "sort_order": 101},
    {"key": "openssh.reconnect_grace_seconds", "group": "OpenSSH升级", "type": "integer", "value": "60",
     "label": "看门狗回滚宽限（秒）",
     "description": "切换 sshd 后平台重新连接验证的宽限时间；超时未确认成功则由看门狗自动恢复旧版本。保存后下次执行生效",
     "sort_order": 102, "min_value": 10, "max_value": 600},
    {"key": "openssh.test_port", "group": "OpenSSH升级", "type": "integer", "value": "2222",
     "label": "预验证备用端口",
     "description": "新二进制先在备用端口拉起并真实连接验证，通过后才切换；0 表示跳过该预验证。保存后下次执行生效",
     "sort_order": 103, "min_value": 0, "max_value": 65535},
    {"key": "openssh.default_configure_opts", "group": "OpenSSH升级", "type": "string",
     "value": "--prefix=/usr --sysconfdir=/etc/ssh --with-pam",
     "label": "默认 configure 参数",
     "description": "向导默认编译参数（--prefix/--sysconfdir 固定为系统布局不可改，其余可追加）。保存后刷新向导生效",
     "sort_order": 104},
    {"key": "openssh.package_max_size_mb", "group": "OpenSSH升级", "type": "integer", "value": "20",
     "label": "源码包上传大小限制 (MB)",
     "description": "OpenSSH 源码包上传上限；上传校验立即按新限制生效",
     "sort_order": 105, "min_value": 1, "max_value": 2048},
    {"key": "system.retention_openssh_task_days", "group": "系统", "type": "integer", "value": "90",
     "label": "OpenSSH 升级任务保留天数",
     "description": "超过天数的 OpenSSH 升级/回滚任务记录将在次日自动清理时删除；0 表示不清理",
     "sort_order": 67, "min_value": 0, "max_value": 3650},
]
